package net.cristichi.ww.updater;
...
public class Updater {
	...
	private boolean versionCheck() {
		...
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(DELIMETER).length >= 2) {
				...
				//I just don't want it to update backwards during testing.
				boolean remoteIsOutdated = true;
				try {
					if (Double.parseDouble(localVersion) > Double.parseDouble(remoteVersion)) {
						remoteIsOutdated = false;
					}
				} catch (NumberFormatException e) {
					// This indicates that I f*ed up the version number. Ok in this case, just
					// ignore the version numbers
				}

				if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion) || !remoteIsOutdated) {
	...
	public boolean shouldUpdate(String localVersion, String remoteVersion) {
		 ...
		 * localVersion.indexOf("MC"))); double mcNew =
		 ...
		 * 
		 * if (mcCurrent<=mcNew && vCurrent < vNew) { return true; } return false;
		 * }catch (NumberFormatException e) { }
		 */
